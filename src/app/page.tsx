...
export default async function Home({ searchParams }: PageProps) {
  try {
    ...
    const [posts, total, categoriesCount] = await Promise.all([
      ...
      // Attachment 테이블이 없어서 발생하는 에러인지 확인하기 위해 에러를 던짐
      throw err;
    })
    ...
    return (
      <div className="p-10 text-red-500 bg-red-50 min-h-screen">
        <h1 className="text-2xl font-bold mb-4">시스템 에러 발생 (상세 로그)</h1>
        <pre className="whitespace-pre-wrap bg-white p-4 rounded border border-red-200">
          {error.stack || error.message || String(error)}
        </pre>
        <div className="mt-4 text-zinc-600">
          이 에러 메시지를 복사해서 알려주시면 즉시 해결해 드리겠습니다.
        </div>
      </div>
    )
  }
